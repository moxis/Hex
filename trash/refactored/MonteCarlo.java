...
class MonteCarlo {
    static ArrayList<int[]> emptyArray = new ArrayList<int[]>();    
    private Game game;
    ...
    MonteCarlo(Game game, AIEngine engine) {
        this.game = game;
        this.engine = engine;
    }

    MonteCarlo(Game game, AIEngine engine, boolean rave, boolean multithread, int thinkingTime) {
        this.game = game;
        this.engine = engine;
        this.rave = rave;
    ...
    public void initializeNode(int[][] state) {
        ...
        if(!nodes.containsKey(stateHash)) {
            engine.setGame(game);
            List<int[]> unexploredMoves = engine.getAvailableMoves();
    ...
    public TreeNode expand(TreeNode node) {
        ...
        engine.setGame(childGame);
        List<int[]> childUnexploredMoves = engine.getAvailableMoves();
    ...
    public Object[] simulate(TreeNode node) {
        ...
        while (winner == 0) {
            engine.setGame(game);
            List<int[]> availableMoves = engine.getAvailableMoves(false, node.numRollouts);
